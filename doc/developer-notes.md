...
- [Development guidelines](#development-guidelines)
    - [General Qnodecoin](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Qnodecoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Qnodecoin is a multi-threaded application, and deadlocks or other
...
Qnodecoin can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Qnodecoin, where everyone uses
...
pay attention to for reviewers of Qnodecoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Qnodecoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Qnodecoin, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Qnodecoin so that they can be integrated
...
file descriptors, it will cause problems with Qnodecoin's `select()` loop, because
...
compatibility between nodes. This happened in Qnodecoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Qnodecoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Qnodecoin, the wallet was always
